...
export default function Voice() {
  ...
  const [selectedKey, setSelectedKey] = useState(preferences?.voiceKey || 'aria');
  ...
  const onPreview = async (voice) => {
    try {
      ...
      const res = await fetch(`${API_URL}/voices/${voice.id}/preview`, { credentials: 'include' });
  ...
  const onContinue = async () => {
    ...
    stopPreview();
    try {
      await patchPreferences({ voiceKey: selected.key, briefingDuration: Number(duration) });
      navigate('/time');
    } catch (err) {
    ...
    <div className="mx-auto flex min-h-screen max-w-md flex-col px-6 pb-32 pt-7">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mt-10">
        <h1 className="text-3xl font-bold leading-tight">
          Pick a <span className="serif-accent text-primary">narrator voice.</span>
        </h1>
        <p className="mt-2 text-sm text-muted">Tap ▶ to hear a 10-second sample.</p>

        <div className="mt-7 space-y-3">
          {isLoading && <div className="h-20 animate-pulse rounded-2xl border border-line bg-card" />}
          {voices.map((v) => (
            <VoiceCard
              key={v.key}
              voice={v}
          ...
          {!isLoading && !voices.length && (
            <div className="rounded-2xl border border-line bg-card p-4 text-sm text-muted">
              No voices loaded — the API server is not reachable or voices failed to seed.
            </div>
          )}
        </div>

        <p className="mt-9 font-mono text-[10px] uppercase tracking-[0.25em] text-muted">· Brief length</p>
        <h2 className="mt-1.5 text-xl font-bold">
          How long is <span className="serif-accent text-primary">your morning?</span>
        </h2>
      ...
      <div className="fixed inset-x-0 bottom-0 mx-auto max-w-md px-6 pb-8">
        <Button full size="lg" loading={saving} onClick={onContinue}>
          Continue with {selected?.name || 'Aria'} · {storiesForDuration(Number(duration))} stories →
        </Button>
      </div>
    </div>
  );
