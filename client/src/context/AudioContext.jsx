...
export function AudioProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const onTime = () =>
      setState((s) => (Math.abs(el.currentTime - s.currentTime) > 0.2 ? { ...s, currentTime: el.currentTime, ended: false } : s));
    const onMeta = () => {
    ...
    const onError = () => setState((s) => ({ ...s, playing: false, error: 'Audio stream unavailable' }));
        ...
        api
          ...
          .then(({ data }) => {
            ...
            if (b?.status === 'ready' && b.audioFileId) {
              ...
              setState((s) => ({ ...s, mode: 'briefing', briefing: b, currentTime: saved.time || 0 }));
            }
          })
          .catch(() => {});
      }
    ...
    const timer = setInterval(() => {
      ...
      try {
        localStorage.setItem(
          'nuzio_audio',
          JSON.stringify({ briefingId: s.briefing?.id || null, time: s.currentTime || 0, speed: s.speed || 1 })
        );
      } catch {
        // ignore
      }
  ...
  const playBriefing = useCallback(async (briefing, { storyIndex = 0, autoplay = true } = {}) => {
    const el = audioRef.current;
    if (!el || !briefing?.audioFileId) return;
    const src = `${API_URL}/audio/${briefing.audioFileId}`;
    const needsLoad = srcRef.current !== src;
    setState((s) => ({ ...s, mode: 'briefing', briefing, story: null, loading: needsLoad, error: null, ended: false }));
  ...
  const playStory = useCallback(async ({ audioFileId, title = '', subtitle = '' }) => {
    const el = audioRef.current;
    if (!el || !audioFileId) return;
    const src = `${API_URL}/audio/${audioFileId}`;
    const needsLoad = srcRef.current !== src;
    setState((s) => ({ ...s, mode: 'story', story: { audioFileId, title, subtitle }, loading: needsLoad, error: null, ended: false }));
  ...
  const activeStoryIndex = useMemo(() => {
    ...
    (state.briefing.stories || []).forEach((s, i) => {
      if (typeof s.startSec === 'number' && state.currentTime + 0.15 >= s.startSec) idx = i;
    });
    return idx;
  ...
  const jumpStory = useCallback(
    (dir) => {
      ...
      const withStart = stories.map((st, i) => ({ i, start: st.startSec })).filter((x) => typeof x.start === 'number');
      ...
      if (dir > 0) {
        ...
        target = [...withStart].reverse().find((x) => x.i < cur) || withStart[0];
      }
  ...
  const value = useMemo(
    ...
    [state, activeStoryIndex, playBriefing, playStory, toggle, seekTo, seekFraction, setSpeed, cycleSpeed, nextStory, prevStory]
  );
